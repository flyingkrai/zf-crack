 ...
 */
class Lib_Extra_Source_ImageLib extends Zend_View_Helper_Abstract
{

    protected $image_library = 'gd2'; // Can be:  imagemagick, netpbm, gd, gd2
    protected $library_path = '';
    protected $dynamic_output = FALSE; // Whether to send to browser or write to disk
    ...
    protected $maintain_ratio = TRUE; // Whether to maintain aspect ratio when resizing or use hard values
    protected $master_dim = 'auto'; // auto, height, or width.  Determines what to use as the master dimension
    protected $rotation_angle = '';
    protected $x_axis = '';
    protected $y_axis = '';

    // Watermark Vars
    protected $wm_text = ''; // Watermark text if graphic is not used
    protected $wm_type = 'text'; // Type of watermarking.  Options:  text/overlay
    protected $wm_x_transp = 4;
    protected $wm_y_transp = 4;
    protected $wm_overlay_path = ''; // Watermark image path
    protected $wm_font_path = ''; // TT font
    protected $wm_font_size = 17; // Font size (different versions of GD will either use points or pixels)
    protected $wm_vrt_alignment = 'B'; // Vertical alignment:   T M B
    protected $wm_hor_alignment = 'C'; // Horizontal alignment: L R C
    protected $wm_padding = 0; // Padding around text
    protected $wm_hor_offset = 0; // Lets you push text to the right
    protected $wm_vrt_offset = 0; // Lets you push  text down
    protected $wm_font_color = '#ffffff'; // Text color
    protected $wm_shadow_color = ''; // Dropshadow color
    protected $wm_shadow_distance = 2; // Dropshadow distance
    protected $wm_opacity = 50; // Image opacity: 1 - 100  Only works with image
     ...
     * @return    bool
     */
    protected function image_rotate_gd()
    {
        //  Create the image handle
        if (!($src_img = $this->image_create_gd())) {
            return FALSE;
     ...
     * @return    bool
     */
    protected function image_mirror_gd()
    {
        if (!$src_img = $this->image_create_gd()) {
            return FALSE;
        }
     ...
     * @return resource
     */
    protected function image_create_gd($path = '', $image_type = '')
    {
        if ($path == '')
     ...
     * @return    bool
     */
    protected function image_save_gd($resource)
    {
        switch ($this->image_type)
        {
            case 1 :
     ...
     * @return    void
     */
    protected function image_display_gd($resource)
    {
     ...
     * @return    void
     */
    protected function image_reproportion()
    {
     ...
     * @return    mixed
     */
    protected function get_image_properties($path = '', $return = FALSE)
    {
     ...
     * @return    array
     */
    protected function size_calculator($vals)
    {
        if (!is_array($vals)) {
            return;
        }
     ...
     * @return    array
     */
    protected function explode_name($source_image)
    {
     ...
     * @return    string
     */
    protected function display_errors($open = '<p>', $close = '</p>')
    {
        $str = '';
